...
GraphQL은 클라이언트 애플리케이션에서 어떤 데이터가 필요한지 기술할 수 있는 쿼리 언어입니다. 대부분의 REST API에서는 실제 클라이언트가 표시하는 형태와 API 프로토콜이 일치하지 않는 경우가 많습니다. (의도적으로 뷰와 독립적으로 설계하는 편입니다.) 이와 달리 GraphQL은 *쿼리를 클라이언트의 UI 계층 구조와 유사하게 구성할 수 있습니다.* 따라서 쿼리가 하나의 커다란 트리를 이룹니다.
...
인자는 스키마에 정의된 대로 받을 수 있습니다. (여기서 GraphQL은 SQL만큼 강력한 쿼리 언어는 아님을 알 수 있습니다. GraphQL 표준에서 실제 반환값의 시맨틱은 정해져 있지 않습니다.)

만약 필드가 객체나 리스트 타입(스칼라가 아닌 타입)을 가진다면, 하위 객체의 실렉션 세트를 반드시 명시해야 합니다.
...
GraphQL 쿼리의 가장 바깥 실렉션 세트는 '쿼리 루트' 객체로부터 시작하도록 정해져 있습니다.
